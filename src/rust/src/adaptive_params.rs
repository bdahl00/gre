    ...
    pub fn update(&mut self, draw: f64, scan_index: usize) {
        if scan_index > 0 {
            ...
            if scan_index == 1 {
                ...
                let new_var = (scan_index - 2) as f64 * (self.variance - self.epsilon)
                    + (scan_index - 1) as f64 * self.mean * self.mean
                    + draw * draw;
                let new_var =
                    (new_var - scan_index as f64 * new_mean * new_mean) / (scan_index - 1) as f64;
